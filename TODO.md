...
- **Token escopado por app externa** — decisão #19 fechada, falta: formato de armazenamento (novo arquivo `~/.warden/tokens.toml`? campo em `config.toml`?), comando CLI de emissão/revogação, validação de escopo no middleware de auth da API.
