...
public sealed class DiscoveryApiTests : IAsyncLifetime
{
    ...
    public async Task ScanPathsRequiresAuth()
    {
        ...
        var response = await unauthenticated.GetAsync("/scan-paths");
    ...
    public async Task AddAndRemoveScanPathRoundTrips()
    {
        ...
        var added = await (await _client.PostAsJsonAsync("/scan-paths", new { path = scanRoot }, JsonOptions))
            .Content.ReadFromJsonAsync<ScanPathsDto>(JsonOptions);
        Assert.Contains(scanRoot, added!.ScanPaths);

        var listed = await _client.GetFromJsonAsync<ScanPathsDto>("/scan-paths", JsonOptions);
        Assert.Contains(scanRoot, listed!.ScanPaths);

        var request = new HttpRequestMessage(HttpMethod.Delete, "/scan-paths")
        {
    ...
    {
        var response = await _client.PostAsJsonAsync("/scan-paths", new { path = "/nope/definitely/not/real" }, JsonOptions);
    ...
    public async Task BrowseListsSubdirectories()
    {
        ...
        var result = await _client.GetFromJsonAsync<BrowseResultDto>($"/browse?path={Uri.EscapeDataString(root)}", JsonOptions);
    ...
    public async Task DiscoverThenPreviewThenApplyRegistersRealProject()
    {
        ...
        await _client.PostAsJsonAsync("/scan-paths", new { path = scanRoot }, JsonOptions);

        var discovered = await _client.GetFromJsonAsync<DiscoverResultDto>("/discover", JsonOptions);
        var found = Assert.Single(discovered!.Projects, p => p.Name == "found-project");
        Assert.Equal("python", found.Type);

        var preview = await (await _client.PostAsJsonAsync("/discover/preview", new { path = found.Path, id = (string?)null }, JsonOptions))
        ...
        var applyResponse = await _client.PostAsJsonAsync("/discover/apply", preview.Config, JsonOptions);
        Assert.Equal(HttpStatusCode.OK, applyResponse.StatusCode);

        // registrado de verdade — /projects lista o projeto recém-aplicado sem restart do processo.
        var projects = await _client.GetFromJsonAsync<JsonElement>("/projects", JsonOptions);
    ...
    public async Task GetProjectConfigReturnsAppliedToml()
    {
        ...
        var preview = await (await _client.PostAsJsonAsync("/discover/preview", new { path = newProjectDir, id = (string?)null }, JsonOptions))
            .Content.ReadFromJsonAsync<ConfigPreviewDto>(JsonOptions);
        await _client.PostAsJsonAsync("/discover/apply", preview!.Config, JsonOptions);

        var config = await _client.GetFromJsonAsync<ProjectConfigDto>($"/projects/{preview.Config.Id}/config", JsonOptions);
    ...
    {
        var response = await _client.GetAsync("/projects/nope/config");
